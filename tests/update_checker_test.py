...
def main():
    assert is_newer_version("2.1.14", "2.1.13")
    assert is_newer_version("2.2", "2.1.99")
    assert not is_newer_version("2.1.14", "2.1.14.0")

    available = check_for_updates("2.1.10", opener=opener_for(manifest()))
    assert available["state"] == "update", available
    current = check_for_updates("2.1.14", opener=opener_for(manifest()))
